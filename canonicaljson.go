...
//
// Three contracts use it. gateway-mqtt-canonical-json/v1 covers runtime-gateway
// MQTT envelopes, evidence-exchange/v1 covers evidence artifacts, and
// commissioned-safety-binding/v1 covers commissioned bindings and firmware
// profiles. They agree on key ordering, separators and string escaping, which
// is what lives here.
//
...
// range is broad, while evidence artifacts and commissioned bindings must
// satisfy the D-011 agreement zone. Numeric policy therefore belongs to the
// caller, and this package accepts only json.Number so a caller cannot leak a
// Go float's formatting into a preimage by accident. A producer that must emit
// D-011 numbers builds them with the d011 subpackage, which is the only way to
// obtain an admissible json.Number and never accepts a bare float64.
//
...
// MarshalWire canonicalises JSON received as bytes.
//
// It validates the Unicode first, then decodes with UseNumber so every number
// keeps the spelling it arrived with, then writes the canonical form. This is
// the operation a verifier needs: reproduce the preimage a producer signed,
// without re-deciding how any number should look.
//
// A producer building a document from typed values does not use this. It
// assembles a map of json.Number values -- see the d011 subpackage for the
// contracts that constrain them -- and calls Marshal.
func MarshalWire(raw []byte) ([]byte, error) {
	if err := ValidateWireUnicode(raw); err != nil {
		return nil, err
	}
	...
	if err := dec.Decode(&payload); err != nil {
		return nil, fmt.Errorf("canonicaljson: decode wire payload: %w", err)
	}
	if dec.More() {
		return nil, fmt.Errorf("canonicaljson: trailing bytes after JSON document")
	}
